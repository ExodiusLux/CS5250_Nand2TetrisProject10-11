...
public class JackTokenizer {
    ...
    public JackTokenizer(File file) {
        ...
        while (scan.hasNextLine()) {
            ...
            while (string_Line.equals("") || has_Comments(string_Line)) {
                if (has_Comments(string_Line)) {
        ...
        while (jack_code.length() > 0) {
            ...
            if (symbols.contains(jack_code.substring(0, 1))) {
                ...
                String strIdentifier = jack_code.substring(0, 1);
                jack_code = jack_code.substring(1);
                while ((Character.isLetter(jack_code.charAt(0))) || (jack_code.substring(0, 1).equals("_"))) {
                    strIdentifier += jack_code.substring(0, 1);
                    jack_code = jack_code.substring(1);
                }
    ...
    public void proceed() {
        if (has_Tokens()) {
            ...
            if (keyWords.contains(current_Item)) {
                ...
                string_Value = current_Item.substring(1, current_Item.length() - 1);
            } 
    ...
    private boolean has_Comments(String string_Line) {
        boolean has_Comments_bool = false;
        if (string_Line.contains("//") || string_Line.contains("/*") || string_Line.startsWith(" *")) {
